...
export function GithubUser({ username }) {
  ...
  const { data, name, loading, error, message, onFetch } =
  ...
  function handleGetUserData() {
    onFetch(); // it doesn't work if we use "onFetch()" directly as click event handler, so we need function
  }

  if (data) {
    return (
      <div>
        <button onClick={handleGetUserData}>Refresh Data</button> <br /> <br />
        <span>
          {name ? (
            <span>
              {username === "ibrahimusluu" ? (
                <span>
                  ...
                  <b style={{ color: "red" }}>Default GitHub Username: </b>{" "}
                  {name},
                </span>
              ) : (
                <span>{name}, </span>
          ...
          <span style={{ color: "red" }}>
            {error && <h1>ERRORRRRRRRRRRRRRRR</h1>}{" "}
